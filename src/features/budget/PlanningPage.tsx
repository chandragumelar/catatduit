...
function TabBudget() {
  ...
  const kategoriKeluar = kategoriMap.keluar.filter(k => !k.id.startsWith('transfer_'))

  const currencies = useMemo(() => {
    const seen = new Set<string>()
    return wallets.filter(w => { if (seen.has(w.currency)) return false; seen.add(w.currency); return true })
  }, [wallets])

  const [activeCurrency, setActiveCurrency] = useState(currencies[0]?.currency ?? 'IDR')
  ...
  const walletIds = useMemo(
    () => new Set(wallets.filter(w => w.currency === activeCurrency).map(w => w.id)),
      ...
      {currencies.length > 1 && (
        <div className={styles.currencyTabs}>
          {currencies.map(w => (
            <button
              key={w.currency}
              className={[styles.currencyTab, activeCurrency === w.currency ? styles.currencyTabActive : ''].join(' ')}
              ...
              <div key={row.id} className={styles.budgetRow}>
                ...
                <div className={styles.budgetProgress}>
                  <div className={styles.budgetBar}>
                    <div
                      className={[styles.budgetBarFill, isOver ? styles.budgetBarOver : ''].join(' ')}
                      style={{ width: `${pct * 100}%` }}
                    />
                  </div>
                  <div className={styles.budgetAmounts}>
                    <span className={isOver ? styles.amountDanger : styles.amountMuted}>
                      {fmt(row.spent, activeCurrency)}
                    </span>
                    <span className={styles.amountMuted}>dari {fmt(row.limit, activeCurrency)}</span>
                  </div>
                </div>
                {editing && (
                  <div className={styles.editRow}>
                    <input
                      ...
                      autoFocus
                    />
                    <button className={styles.editSaveBtn} onClick={() => saveEdit(row.id)}><Check size={16} /></button>
                    <button className={styles.editCancelBtn} onClick={cancelEdit}><X size={16} /></button>
                  </div>
                )}
              </div>
            )
            ...
            return (
              <div key={row.id} className={styles.budgetRow}>
                <div className={styles.budgetRowTop}>
                  ...
                  <span className={styles.budgetRowNama}>{row.nama}</span>
                  {!editing && (
                    <button className={styles.addBtn} onClick={() => startEdit(row.id)}>
...
function TabTagihan() {
  ...
  function openAdd() {
    setEditingTagihan(null); setFormNama(''); setFormNominal(''); setFormTanggal(1)
    setFormWallet(wallets[0]?.id ?? ''); setFormKategori('lainnya_keluar')
    setFormRecurring(true); setShowForm(true)
  }
  ...
  function handleSave() {
    ...
    let next: Tagihan[]
    if (editingTagihan) {
      next = tagihan.map(t => t.id === editingTagihan.id
      ...
      next = [...tagihan, {
        id: generateId(), nama: formNama.trim(), nominal, tanggal: formTanggal,
        wallet_id: formWallet, kategori: formKategori, isRecurring: formRecurring, paidMonths: [],
      }]
  ...
  function togglePaid(id: string) {
    const next = tagihan.map(t => {
      ...
      return { ...t, paidMonths: isPaid ? t.paidMonths.filter(m => m !== currentMonth) : [...t.paidMonths, currentMonth] }
    })
  ...
  function handleDelete(id: string) {
    ...
    saveTagihan(next); setTagihanState(next); setConfirmDeleteId(null)
    ...
    <div className={styles.tabContent}>
      ...
      {unpaid.length > 0 && (
        <div className={styles.section}>
          ...
          {unpaid.map(t => {
            ...
            return (
              <div key={t.id} className={[styles.tagihanRow, over ? styles.tagihanRowOverdue : soon ? styles.tagihanRowSoon : ''].join(' ')}>
                <button className={styles.checkCircle} onClick={() => togglePaid(t.id)}>
                ...
                <div className={styles.tagihanInfo} onClick={() => openEdit(t)}>
                  ...
                  <div className={styles.tagihanMeta}>
                    <span className={[styles.tagihanDue, over ? styles.metaDanger : soon ? styles.metaWarn : ''].join(' ')}>
                      {(over || soon) ? <AlertCircle size={12} strokeWidth={2} /> : <Calendar size={12} strokeWidth={1.5} />}
      ...
      {paid.length > 0 && (
        <div className={styles.section}>
          ...
          {paid.map(t => (
            <div key={t.id} className={[styles.tagihanRow, styles.tagihanRowPaid].join(' ')}>
              <button className={styles.checkCircle} onClick={() => togglePaid(t.id)}>
              ...
              <div className={styles.tagihanRight}>
                <div className={[styles.tagihanNominal, styles.tagihanNominalPaid].join(' ')}>{fmt(t.nominal, getWalletCurrency(t.wallet_id))}</div>
                <button className={styles.deleteBtn} onClick={() => setConfirmDeleteId(t.id)}>
                  <Trash2 size={14} strokeWidth={1.5} />
                </button>
              </div>
      ...
      {tagihan.length === 0 && (
        <div className={styles.emptyState}>
          ...
          <div className={styles.emptyDesc}>Tambah tagihan rutin seperti listrik, sewa, atau langganan biar kamu tahu kapan harus bayar.</div>
        </div>
      )}
          ...
          <div className={styles.sheet} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.sheetBody}>
              <div className={styles.field}>
                ...
                <input className={styles.fieldInput} type="text" placeholder="cth. Sewa kos, Listrik PLN"
                  value={formNama} onChange={e => setFormNama(e.target.value)} maxLength={50} autoFocus />
              </div>
              <div className={styles.field}>
                ...
                <input className={styles.fieldInput} type="text" inputMode="numeric" placeholder="0"
                  value={formNominal} onChange={e => setFormNominal(fmtInput(e.target.value))} />
              </div>
              ...
              <div className={styles.field}>
                ...
                <div className={styles.selectWrap}>
                  <select className={styles.fieldSelect} value={formKategori} onChange={e => setFormKategori(e.target.value)}>
                    {kategoriMap.keluar.filter(k => !k.id.startsWith('transfer_')).map(k =>
              ...
              <label className={styles.toggleRow}>
                ...
                <button role="switch" aria-checked={formRecurring}
                  className={[styles.toggle, formRecurring ? styles.toggleOn : ''].join(' ')}
...
function TabTabungan() {
  ...
  function handleDelete(id: string) {
    ...
    saveGoals(next); setGoalsState(next); setConfirmDeleteId(null)
    ...
    <div className={styles.tabContent}>
      ...
      {goals.length === 0 ? (
        <div className={styles.emptyState}>
          ...
          <div className={styles.emptyDesc}>Buat tujuan menabung biar lebih terarah — liburan, gadget, atau apapun yang kamu inginkan.</div>
        </div>
      ) : (
        <div className={styles.section}>
          {goals.map(goal => {
      ...
      {showForm && (
        <div className={styles.overlay} onClick={resetForm}>
          <div className={styles.sheet} onClick={e => e.stopPropagation()}>
            <div className={styles.sheetHandle} />
            <div className={styles.sheetHeader}>
              <h2 className={styles.sheetTitle}>{editingGoal ? 'Edit Target' : 'Tambah Target'}</h2>
              <button className={styles.sheetClose} onClick={resetForm}><X size={20} /></button>
            </div>
            <div className={styles.sheetBody}>
              <div className={styles.iconPicker}>
                {ICONS.map(ic => (
                  <button key={ic} className={[styles.iconOption, formIcon === ic ? styles.iconOptionActive : ''].join(' ')} onClick={() => setFormIcon(ic)}>
                    {ic}
                  </button>
                ))}
              </div>
              <div className={styles.field}>
                ...
                <input className={styles.fieldInput} type="text" placeholder="cth. Liburan Bali, Laptop baru"
                  value={formNama} onChange={e => setFormNama(e.target.value)} maxLength={50} autoFocus />
              </div>
              <div className={styles.field}>
    ...
    <div className={styles.page}>
      ...
      <div className={styles.tabBar}>
        {(['budget', 'tagihan', 'tabungan'] as Tab[]).map(tab => (
          ...
          >
            {tab === 'budget' ? 'Budget' : tab === 'tagihan' ? 'Tagihan' : 'Tabungan'}
          </button>
        ))}
      </div>
