...
// Transfer dari wallet secondary ke wallet base (atau sebaliknya)
// nominalFrom: nominal dalam currency wallet asal
// nominalTo: nominal dalam currency wallet tujuan (dihitung dari rate)

function saveCrossCurrencyTransfer({ fromWalletId, toWalletId, nominalFrom, tanggal, catatan = '' }) {
  if (!fromWalletId || !toWalletId || fromWalletId === toWalletId) {
    showToast('❌ Pilih dua wallet yang berbeda.'); return false;
  }
  if (!nominalFrom || nominalFrom <= 0) { showToast('❌ Nominal tidak valid.'); return false; }

  const fromWallet = getWalletById(fromWalletId);
  const toWallet   = getWalletById(toWalletId);
  const rate       = getExchangeRate();

  // Hitung nominal di currency tujuan
  const fromCurrency = getWalletCurrency(fromWallet);
  const toCurrency   = getWalletCurrency(toWallet);
  const baseCurrency = getBaseCurrency();

  let nominalTo;
  if (fromCurrency !== baseCurrency) {
    // secondary → base
    nominalTo = Math.round(nominalFrom * rate);
  } else {
    // base → secondary
    nominalTo = Math.round(nominalFrom / rate);
  }
  ...
  const txOut = {
    id: generateId(), jenis: 'keluar',
    nominal: Math.min(nominalFrom, MAX_NOMINAL),
    kategori: 'transfer_keluar', tanggal: txDate,
    catatan: catatan || `Transfer ke ${toWallet.nama}`,
    wallet_id: fromWalletId, timestamp: now,
    type: 'transfer_out', group_id: groupId, peer_wallet_id: toWalletId,
    is_cross_currency: true, cross_rate: rate,
  };

  const txIn = {
    id: generateId(), jenis: 'masuk',
    nominal: Math.min(nominalTo, MAX_NOMINAL),
    kategori: 'transfer_masuk', tanggal: txDate,
    catatan: catatan || `Transfer dari ${fromWallet.nama}`,
    wallet_id: toWalletId, timestamp: now + 1,
    type: 'transfer_in', group_id: groupId, peer_wallet_id: fromWalletId,
    is_cross_currency: true, cross_rate: rate,
  };
